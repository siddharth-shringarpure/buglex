...
\geometry{a4paper, left=0.6in, right=0.6in, top=0.75in, bottom=0.75in}
...
\hypersetup{
    pdftitle={semlex: Semantic-Lexical Fusion for Performance Bug Report Classification},
...
\title{semlex: Semantic-Lexical Fusion for Performance Bug Report Classification}
\author{Siddharth Shringarpure\,\orcidlink{0009-0002-7222-4946}}
\date{\textcolor{darkgray}{Last updated: \today\ $\cdot$ v1.0.1} \\[2pt]
\small \textcolor{darkgray}{\texttt{https://doi.org/10.5281/zenodo.20937645}}}
...
\begin{abstract}
    As issue volumes outpace manual triage in software engineering, automated classification of bug reports becomes increasingly important for routing issues to the right maintainers to facilitate timely issue resolution. This report evaluates a hybrid pipeline that combines sparse TF-IDF features with dense sentence embeddings for classifying performance-related bug reports, benchmarked against a TF-IDF-based Naïve Bayes baseline across five machine-learning repositories, where reports are either performance relevant (eg: accuracy, inference speed) or not. A hybrid Logistic Regression model achieves higher macro-F1 across all datasets. Ablations show that most gains still hold when the sentence embedding dimension is reduced, whilst common preprocessing steps, such as stop-word removal and text lemmatisation, add little-to-no benefit.
\end{abstract}
...
\begin{enumerate}[label=(\arabic*)]
    \item a reproducible hybrid text-classification pipeline; and
    \item a paired, multi-project evaluation examining the source of observed performance gains.
\end{enumerate}
The nature of performance bug reports motivates a hybrid representation. On one hand, they contain project-specific lexical signals, such as version numbers, configuration flags, and API identifiers, whose discriminative value lies in exact-token presence rather than meaning; these are precisely the cues that sparse TF-IDF captures and that dense encoders, trained for semantic similarity, tend to underweight. On the other hand, the natural language \textit{description} of a performance problem is lexically inconsistent across bug reporters --- ``slow'', ``takes forever'', ``relatively high latency'' all denote the same underlying class --- which is the case that semantic sentence embedding representation handles well and exact-match methods do not. Neither representation alone is sufficient: lexical models miss alternative phrasings, and semantic models underweight the project-specific tokens that often decide the label.


\section{Related Work}
Classical bug and issue classification has largely relied on sparse lexical features, such as TF-IDF, paired with conventional classifiers like NB and linear VM~\cite{McCallum1998ACO,fan2008liblinear}. These approaches have been applied to related tasks, including GitHub issue-type prediction~\cite{kallis_predicting_2021} (bug report vs feature request vs question), but are less effective at capturing semantic similarity between differently-worded reports.
More recent work therefore shifts towards semantic representations. Sentence embeddings map reports with similar meanings closer together in representation space, making them useful for classification and retrieval~\cite{reimers_sentence-bert_2019}, although they introduce additional computational cost and remain constrained by encoder input length~\cite{reimers_sentence-bert_2019,nussbaum_nomic_2025}.
Techniques such as Matryoshka Representation Learning (MRL) take this further by training encoders to preserve information at multiple granularities within a single nested embedding, meaning that embeddings can be truncated with limited accuracy loss~\cite{kusupati2022matryoshka}. Attention-based architectures have also been applied to distinguish non-functional bug reports from other issues~\cite{long2022multifacetedhierarchicalreportidentification}; however, they impose a parameter count disproportionate to the available training data at the scale considered here, where the smallest dataset contains 286 reports with 33 positive instances. 
The gap in existing work is therefore not an absence of lexical or semantic methods individually, but rather the lack of a directly compared, feature-level sparse--dense fusion evaluated under paired conditions for performance bug classification specifically.
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{macro_f1_heatmap.png}
...
\input{tables/wilcoxon_results.tex}
\caption{Paired Wilcoxon results for \texttt{Hybrid LogReg} vs \texttt{Baseline NB}.}\label{tab:wilcoxon-results}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{hybrid_ablation.png}
...
\resizebox{\linewidth}{!}{\input{tables/efficiency_summary.tex}}
\end{minipage}%
}
...
\small
\setlength{\tabcolsep}{6pt}
\input{tables/preproc_ablation.tex}
...
The repository, available at: \href{https://github.com/siddharth-shringarpure/semlex}{\texttt{github.com/siddharth-shringarpure/semlex}}, contains the full source code, datasets, and raw result CSV files used to generate the tables and figures in the report. Supporting documentation --- including the
\href{https://github.com/siddharth-shringarpure/semlex/blob/main/replication.pdf}{\texttt{replication guide}}, 
\href{https://github.com/siddharth-shringarpure/semlex/blob/main/manual.pdf}{\texttt{manual}}, and 
\href{https://github.com/siddharth-shringarpure/semlex/blob/main/requirements.pdf}{\texttt{requirements}} --- is available under
\href{https://github.com/siddharth-shringarpure/semlex/tree/main/docs}{\texttt{docs/}}. Source code is under
\href{https://github.com/siddharth-shringarpure/semlex/tree/main/src}{\texttt{src/}}, datasets under 
\href{https://github.com/siddharth-shringarpure/semlex/tree/main/datasets}{\texttt{datasets/}}, and generated outputs under 
\href{https://github.com/siddharth-shringarpure/semlex/tree/main/results}{\texttt{results/}}.
