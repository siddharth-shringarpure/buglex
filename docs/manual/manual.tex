...
\begin{enumerate}
      ...
      \item run the default experiment with
            \texttt{uv run python -m src.run\_experiments -{}-with-plots}
      \item if needed, run the full preprocessing ablation with
            \texttt{uv run python -m src.run\_experiments --all-preprocessing -{}-with-plots}
      \item rebuild the report with
            \texttt{uv run python -m src.tools.build\_docs -{}-report-only}
\end{enumerate}
